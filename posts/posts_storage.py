...
def process_response(response_obj):
    ...
    for item in response_list:
        ...
        if not attachments:
            continue
        # If more then one photo - skip this post. Cant process such posts yet
        ...
        if len(string_of_links) > 0:
            new_item = dict(date=datetime.fromtimestamp(item.get('date')), comments_amount=item.get('comments').get(
                'count'), likes_amount=item.get('likes').get('count'), reposts_amount=item.get('reposts').get('count'), img_links=string_of_links)
...
def get_posts(group_domain, posts_amount=100):
    ...
    response = requests.get(req_url, params=payload)
    list_of_posts = process_response(response)
